...
import io.throneproj.thronem.core.matchedOutbound
import io.throneproj.thronem.fmt.TAG_DIRECT
import io.throneproj.thronem.fmt.TrafficNode
...
class OutboundTrafficAggregator(private val graph: Map<String, TrafficNode> = emptyMap()) {
    ...
    fun onEvents(events: ConnectionEvents) {
        if (events.reset) {
            val liveIds = events.iterator().let { iterator ->
                buildSet {
                    while (iterator.hasNext()) add(iterator.next().id)
                }
            }
    ...
    private fun Connection.carriers(matched: String): Set<Long> {
        // chain runs from the dialing hop up to the matched outbound, so reading
        // it backwards says which member each selector on the way resolved to.
        val chain = chain().toList()
        val resolvedByChain = HashMap<String, String>(chain.size)
        for (index in 1..chain.lastIndex) {
            resolvedByChain[chain[index]] = chain[index - 1]
        }
