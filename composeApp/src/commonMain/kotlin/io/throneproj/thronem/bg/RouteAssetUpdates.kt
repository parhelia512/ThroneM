...
import io.throneproj.thronem.ktx.unTarGzWithoutDirTo
...
internal suspend fun updateSingleRouteAsset(
    ...
    }.execute().writeToWithProgress(
        targetFile.absolutePath,
        object : HTTPResponseWriteToProgressHandler {
            override fun update(written: Long, total: Long) {
                if (total <= 0) return
                updateProgress(((written.toDouble() / total) * 100.0).toFloat())
            }
        },
    )
...
internal abstract class AssetsUpdater(
    ...
    protected val remoteSource: RemoteSource = remoteSource ?: object : RemoteSource {
        override fun fetchString(url: String): String = newRequest(url).execute().content.value
    }
    ...
    override suspend fun performUpdate(updates: List<UpdateInfo>) {
        ...
        try {
            ...
            for (file in cacheFiles) {
                file.tryUnpackTo(destinationDir)
            }
...
internal class GithubAssetUpdater(
    ...
    override suspend fun performUpdate(updates: List<UpdateInfo>) {
        ...
        try {
            ...
            for (file in cacheFiles) {
                file.unTarGzWithoutDirTo(destinationDir)
...
internal class GithubReleaseZipUpdater(
    ...
    override suspend fun performUpdate(updates: List<UpdateInfo>) {
        ...
        try {
            updateProgress(10f)
            newRequest(url).execute().writeTo(cacheFile.absolutePath)
            updateProgress(60f)
            cacheFile.tryUnpackTo(destinationDir)
            updateProgress(25f)
