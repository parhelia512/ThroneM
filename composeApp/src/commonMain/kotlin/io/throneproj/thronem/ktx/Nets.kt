...
import io.throneproj.thronem.fmt.LOCALHOST4
import io.throneproj.thronem.fmt.LOCALHOST_NAME
...
suspend fun localProxyURL(scheme: String): URL = newURL(scheme).apply {
    host = LOCALHOST4
...
val USER_AGENT by lazy { "thronem/${BuildConfig.VERSION_NAME} (sing-box ${Libbox.version()})" }
...
fun generateUserAgent(userAgent: String): String {
    ...
    return userAgent.replace($$"$version", BuildConfig.VERSION_NAME)
        .replace($$"$box_version", Libbox.version())
}
