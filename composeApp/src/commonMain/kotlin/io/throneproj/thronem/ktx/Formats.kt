...
import io.throneproj.thronem.fmt.anytls.parseAnyTLS
import io.throneproj.thronem.fmt.http.parseHttp
import io.throneproj.thronem.fmt.hysteria.parseHysteria1
import io.throneproj.thronem.fmt.hysteria.parseHysteria2
import io.throneproj.thronem.fmt.juicity.parseJuicity
import io.throneproj.thronem.fmt.masque.parseMasque
import io.throneproj.thronem.fmt.naive.parseNaive
import io.throneproj.thronem.fmt.parseUniversal
import io.throneproj.thronem.fmt.shadowsocks.parseShadowsocks
import io.throneproj.thronem.fmt.socks.parseSOCKS
import io.throneproj.thronem.fmt.trojan.parseTrojan
import io.throneproj.thronem.fmt.tuic.parseTuic
import io.throneproj.thronem.fmt.v2ray.parseV2Ray
import io.throneproj.thronem.fmt.wireguard.parseWireGuardUri
...
suspend fun parseProxies(text: String): List<AbstractBean> {
    ...
    suspend fun String.parseLink(entities: MutableList<AbstractBean>) {
        if (startsWith("sing-box://import-remote-profile?") || startsWith("thronem://subscription?")) {
        ...
        when (scheme) {
            ...
            "wireguard", "wireguard-go" -> {
                ...
                runCatching {
                    entities.add(parseWireGuardUri(this))
                }.onFailure {
                    Logs.w(it)
                }
            }
            ...
            "masque" -> {
                Logs.d("Try parse masque link: $this")
                runCatching {
...
private val PRE_RELEASE_PATTERN = Regex("[.-](alpha|beta|rc|dev|pre|snapshot)", RegexOption.IGNORE_CASE)

/** Replaces libcore's isPreRelease: true for pre-release version tags. */
fun isPreReleaseVersion(version: String): Boolean {
    return PRE_RELEASE_PATTERN.containsMatchIn(version)
}
