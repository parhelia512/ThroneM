...
import io.throneproj.thronem.ui.SnackbarEmitterEffect
...
fun rememberProfilePickerState(
    ...
    LaunchedEffect(preSelected) {
        val initialGroup = withContext(Dispatchers.IO) {
            preSelected.takeIf { it != null && it > 0 }?.let { ProfileManager.getProfile(it)?.groupId }
        }
        selectedGroup = initialGroup ?: DataStore.currentGroupId()
        viewModel.scrollToProxy(preSelected ?: 0L)
    }
