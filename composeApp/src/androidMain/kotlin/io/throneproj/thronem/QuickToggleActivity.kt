...
class QuickToggleActivity : Activity() {
    ...
    private suspend fun toggle() {
        ...
        when {
            state.canStop -> {
                if (profileId == DataStore.selectedProxySet.get() || profileId == NO_PROFILE_ID) {
                    resolveRepository().stopService()
                } else {
                    DataStore.selectedProxySet.set(profileId)
                    resolveRepository().reloadService()
                }
            }

            state == ServiceState.Stopped || state == ServiceState.Idle -> {
                if (profileId >= 0L) DataStore.selectedProxySet.set(profileId)
